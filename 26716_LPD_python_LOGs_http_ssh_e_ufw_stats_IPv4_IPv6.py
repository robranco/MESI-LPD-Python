...
<head>
    ...
    <style>
        ...
        canvas {{ width: 320px; height: 320px; max-width: 100%; }}
        ul {{ list-style: none; padding: 0; }}
        li {{ margin: 0.25rem 0; }}
        .legenda {{ display: flex; justify-content: center; gap: 1rem; margin-top: 0.5rem; font-size: 0.95rem; }}
        .legenda span {{ display: inline-flex; align-items: center; gap: 0.4rem; }}
        .legenda i {{ width: 12px; height: 12px; display: inline-block; border-radius: 50%; }}
    </style>
</head>
<body>
    ...
    <section class="graficos">
        <div class="grafico">
            ...
            <canvas id="graficoTotal" width="320" height="320"></canvas>
            <div class="legenda">
                <span><i style="background:{dados_totais["cores"][0]};"></i>IPv4 ({contexto['ipv4_total']})</span>
                <span><i style="background:{dados_totais["cores"][1]};"></i>IPv6 ({contexto['ipv6_total']})</span>
            </div>
        </div>
        <div class="grafico">
            <h2>Distribuicao de IPs unicos</h2>
            <canvas id="graficoUnico" width="320" height="320"></canvas>
            <div class="legenda">
                <span><i style="background:{dados_unicos["cores"][0]};"></i>IPv4 ({contexto['ipv4_unicos']})</span>
                <span><i style="background:{dados_unicos["cores"][1]};"></i>IPv6 ({contexto['ipv6_unicos']})</span>
            </div>
        </div>
        <div class="grafico">
            <h2>IPv4 unicos x total</h2>
            <canvas id="graficoIPv4Unico" width="320" height="320"></canvas>
            <div class="legenda">
                <span><i style="background:{dados_ipv4_unicos_total["cores"][0]};"></i>Unicos ({contexto['ipv4_unicos']})</span>
                <span><i style="background:{dados_ipv4_unicos_total["cores"][1]};"></i>Repetidos ({ipv4_repetidos})</span>
            </div>
        </div>
        <div class="grafico">
            <h2>IPv6 unicos x total</h2>
            <canvas id="graficoIPv6Unico" width="320" height="320"></canvas>
            <div class="legenda">
                <span><i style="background:{dados_ipv6_unicos_total["cores"][0]};"></i>Unicos ({contexto['ipv6_unicos']})</span>
                <span><i style="background:{dados_ipv6_unicos_total["cores"][1]};"></i>Repetidos ({ipv6_repetidos})</span>
            </div>
        </div>
    </section>
    ...
    <script>
        function desenharPizza(canvasId, valores, cores) {{
        ...
        document.addEventListener('DOMContentLoaded', () => {{
            const dadosTotal = {json.dumps(dados_totais)};
            const dadosUnico = {json.dumps(dados_unicos)};
            const dadosIPv4Unico = {json.dumps(dados_ipv4_unicos_total)};
            const dadosIPv6Unico = {json.dumps(dados_ipv6_unicos_total)};
            desenharPizza('graficoTotal', dadosTotal.valores, dadosTotal.cores);
            desenharPizza('graficoUnico', dadosUnico.valores, dadosUnico.cores);
            desenharPizza('graficoIPv4Unico', dadosIPv4Unico.valores, dadosIPv4Unico.cores);
            desenharPizza('graficoIPv6Unico', dadosIPv6Unico.valores, dadosIPv6Unico.cores);
        }});
    </script>
</body>
</html>
...
def main(argv=None):
    ...
    caminho_relatorio_html = os.path.join(PASTA_RELATORIOS, f"{nome_base}_relatorio_http_ssh_ufw-stats-IPv4-IPv6.html")
